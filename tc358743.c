				     ...
				     struct v4l2_dv_timings *timings)
{
	...
	if (no_signal(sd)) {
		pr_info("[[TC358743-hainh]%s: no valid signal\n", __func__);
		return -ENOLINK;
	}
	if (no_sync(sd)) {
		pr_info("[[TC358743-hainh]%s: no sync on signal\n", __func__);
		return -ENOLCK;
	}
...
static void tc358743_enable_edid(struct v4l2_subdev *sd)
{
	...
	if (state->edid_blocks_written == 0) {
		pr_info("[[TC358743-hainh]%s: no EDID -> no hotplug\n", __func__);
		return;
	}
...
static inline void enable_stream(struct v4l2_subdev *sd, bool enable)
{
	...
	pr_info("[TC358743-hainh]%s: %sable\n",
...
static void tc358743_set_pll(struct v4l2_subdev *sd)
{
	...
	if ((pllctl0 != pllctl0_new) || ((pllctl1 & MASK_PLL_EN) == 0)) {
		...
		pr_info("[[TC358743-hainh]%s: updating PLL clock\n", __func__);
...
static void tc358743_set_csi_mbus(struct v4l2_subdev *sd)
{
	struct tc358743_state *state = to_state(sd);
	...
	case MEDIA_BUS_FMT_UYVY8_1X16:
		pr_info("[[TC358743-hainh]%s: YCbCr 422 16-bit\n", __func__);
	...
	case MEDIA_BUS_FMT_RGB888_1X24:
		pr_info("[[TC358743-hainh]%s: RGB 888 24-bit\n", __func__);
		i2c_wr8_and_or(sd, VOUT_SET2,
				~(MASK_SEL422 | MASK_VOUT_422FIL_100) & 0xff,
				0x00);
		i2c_wr8_and_or(sd, VI_REP, ~MASK_VOUT_COLOR_SEL & 0xff,
		...
		break;
	default:
		pr_info("[[TC358743-hainh]%s: Unsupported format code 0x%x\n",
...
static void tc358743_format_change(struct v4l2_subdev *sd)
{
	...
	if (tc358743_get_detected_timings(sd, &timings)) {
		...
		pr_info("[[TC358743-hainh]%s: Format changed. No signal\n",
				__func__);
	} else {
...
static void tc358743_enable_interrupts(struct v4l2_subdev *sd,
		bool cable_connected)
{
	pr_info("[[TC358743-hainh]%s: cable connected = %d\n", __func__,
			cable_connected);
		...
		bool *handled)
{
	...
	pr_info("[TC358743-hainh]%s: AUDIO_INT = 0x%02x\n", __func__, audio_int);
		...
		bool *handled)
{
	...
	pr_info("[TC358743-hainh]%s: MISC_INT = 0x%02x\n", __func__, misc_int);
		...
		bool *handled)
{
	...
	pr_info("[TC358743-hainh]%s: CBIT_INT = 0x%02x\n", __func__, cbit_int);

	if (cbit_int & MASK_I_CBIT_FS) {

		pr_info("[[TC358743-hainh]%s: Audio sample rate changed\n",
				__func__);
		tc358743_s_ctrl_audio_sampling_rate(sd);
	...
	if (cbit_int & (MASK_I_AF_LOCK | MASK_I_AF_UNLOCK)) {
		...
		pr_info("[[TC358743-hainh]%s: Audio present changed\n",
				__func__);
		tc358743_s_ctrl_audio_present(sd);
...
static void tc358743_hdmi_clk_int_handler(struct v4l2_subdev *sd, bool *handled)
{
	...
	pr_info("[TC358743-hainh]%s: CLK_INT = 0x%02x\n", __func__, clk_int);

	if (clk_int & (MASK_I_IN_DE_CHG)) {

		pr_info("[[TC358743-hainh]%s: DE size or position has changed\n",
				__func__);
...
static void tc358743_hdmi_sys_int_handler(struct v4l2_subdev *sd, bool *handled)
{
	...
	pr_info("[TC358743-hainh]%s: SYS_INT = 0x%02x\n", __func__, sys_int);

	if (sys_int & MASK_I_DDC) {
		bool tx_5v = tx_5v_power_present(sd);

		pr_info("[[TC358743-hainh]%s: Tx 5V power present: %s\n",
	...
	if (sys_int & MASK_I_DVI) {
		pr_info("[[TC358743-hainh]%s: HDMI->DVI change detected\n",
				__func__);
	...
	if (sys_int & MASK_I_HDMI) {
		pr_info("[[TC358743-hainh]%s: DVI->HDMI change detected\n",
				__func__);
...
static int tc358743_isr(struct v4l2_subdev *sd, u32 status, bool *handled)
{
	...
	pr_info("[[TC358743-hainh]%s: IntStatus = 0x%04x\n", __func__, intstatus);
...
static int tc358743_g_input_status(struct v4l2_subdev *sd, u32 *status)
{
	...
	*status |= no_sync(sd) ? V4L2_IN_ST_NO_SYNC : 0;
	...
	pr_info("[[TC358743-hainh]%s: status = 0x%x\n", __func__, *status);
				 ...
				 struct v4l2_dv_timings *timings)
{
	...
	if (v4l2_match_dv_timings(&state->timings, timings, 0)) {
		pr_info("[[TC358743-hainh]%s: no change\n", __func__);
		return 0;
	}

	if (!v4l2_valid_dv_timings(timings,
				&tc358743_timings_cap, NULL, NULL)) {
		pr_info("[[TC358743-hainh]%s: timings out of range\n", __func__);
		return -ERANGE;
	}
		...
		struct v4l2_subdev_format *format)
{
	...
	pr_info("[[TC358743-hainh]%s return at format->pad=%d\n",__func__,format->pad);
	...
	// pr_info("[[TC358743-hainh]%s: format->format.code= 0x%04x\n",__func__,format->format.code);
	// pr_info("[[TC358743-hainh]%s: format->format.width= %0d\n",__func__,format->format.width);
	// pr_info("[[TC358743-hainh]%s: format->format.height= %0d\n",__func__,format->format.height);
	// pr_info("[[TC358743-hainh]%s: format->format.field= 0x%04x\n",__func__,format->format.field);
	// pr_info("[[TC358743-hainh]%s: format->format.colorspace= %0d\n",__func__,format->format.colorspace);
	return 0;
}
		...
		struct v4l2_subdev_format *format)
{
	...
	if (ret){
		pr_info("[TC358743-hainh]-%s exit with tc358743_get_fmt= %04x \n",__func__,ret);
		return ret;
	}
	...
	if (format->which == V4L2_SUBDEV_FORMAT_TRY){
		pr_info("[TC358743-hainh]-%s exit with format->which=V4L2_SUBDEV_FORMAT_TRY\n",__func__);
		return 0;
	}
				...
				struct v4l2_subdev_edid *edid)
{
	...
	pr_info("[[TC358743-hainh]%s, pad %d, start block %d, blocks %d\n",
...
// static int tc358743_g_mbus_fmt(struct v4l2_subdev *sd, struct v4l2_mbus_framefmt *mf)
// {
// 	struct tc358743_state *state = to_state(sd);
// 	u8 vi_rep = i2c_rd8(sd, VI_REP);
...
static int tc358743_probe_of(struct tc358743_state *state)
{
	...
	state->pdata.enable_hdcp    = 0;
	state->pdata.fifo_level     = 0x154;
			  ...
			  const struct i2c_device_id *id)
{
	...
	pr_info("[TC358743-hainh]: Probing hdmi2csi bridge.\n");
	...
	sd = &state->sd;
	sd->dev = &client->dev;
	sd->internal_ops = &tc358743_subdev_internal_ops;
